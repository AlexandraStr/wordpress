		...
		?>
			...
			for ($i=1; $i <= 5 ; $i++) { 
				 if (get_locale()=="en_US") {
                    ...
                    $slider_title = of_get_option('slider_title_en' . $i);
                    $slider_desc = of_get_option('slider_desc_en' . $i);
                    $slider_button_text = of_get_option('slider_button_text_en' . $i);
                    $slider_button_link = of_get_option('slider_button_link_en' . $i);
                } else {
                    $slider_image = of_get_option('slider_image' . $i);
...
function accesspress_breadcrumbs() {
    ...
    $delimiter = '&sol;';
    if (get_locale()=="en_US" ){
        $home = __('Home', 'accesspress-root'); // text for the 'Home En' link
    } else {
        $home = __('Головна', 'accesspress-root'); // text for the 'Home Uk' link
    }
