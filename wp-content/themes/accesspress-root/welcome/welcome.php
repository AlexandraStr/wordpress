<?php
	if(!class_exists('Accesspress_Root_Welcome')) :

		class Accesspress_Root_Welcome {
			...
			public function __construct() {
				...
				$this->req_plugins = array(
					...
					'instant-demo-importer' => array(
						...
						'name' => __('Instant Demo Importer', 'accesspress-root'),
						...
						'info' => __('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'accesspress-root'),
					),
				...
				$this->tab_sections = array(
					...
					'more_wp_stuff' => __('More WordPress Stuff', 'accesspress-root'),
				);

				/** List of Recommended Free Plugins **/
				$this->free_plugins = array(

					'ap-contact-form' => array(
						'slug' => 'ap-contact-form',
						'filename' => 'ap-contact-form.php',
					...
					'accesspress-twitter-auto-post' => array(
						'slug' => 'accesspress-twitter-auto-post',
						'filename' => 'accesspress-twitter-auto-post.php',
						'class' => 'ATAP_Class'
					),
				);
				...
				add_action( 'load-themes.php', array( $this, 'accesspressroot_activation_admin_notice' ) );

				/* Create a Welcome Page */
				add_action( 'admin_menu', array( $this, 'accesspressroot_welcome_register_menu' ) );

				/* Enqueue Styles & Scripts for Welcome Page */
				add_action( 'admin_enqueue_scripts', array( $this, 'accesspressroot_welcome_styles_and_scripts' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_accesspressroot_plugin_installer', array( $this, 'accesspressroot_plugin_installer_callback' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_accesspressroot_plugin_offline_installer', array( $this, 'accesspressroot_plugin_offline_installer_callback' ) );

				/** Plugin Activation Ajax **/
				add_action( 'wp_ajax_accesspressroot_plugin_activation', array( $this, 'accesspressroot_plugin_activation_callback' ) );

				/** Plugin Activation Ajax (Offline) **/
				add_action( 'wp_ajax_accesspressroot_plugin_offline_activation', array( $this, 'accesspressroot_plugin_offline_activation_callback' ) );
			...
			public function get_required_plugin_notification() {
				...
				$accesspressroot_plugin_installed_notif = get_option('accesspressroot_plugin_installed_notif');
				...
				update_option('accesspressroot_plugin_installed_notif', $notif_counter);
				return $notif_counter;
			}

			/** Welcome Message Notification on Theme Activation **/
			public function accesspressroot_activation_admin_notice() {
				global $pagenow;
			...
			public function accesspressroot_welcome_register_menu() {
				$action_count = get_option('accesspressroot_plugin_installed_notif');
				$title        = $action_count > 0 ? esc_html__( 'Welcome', 'accesspress-root' ) : esc_html__( 'Welcome', 'accesspress-root' );
				add_theme_page( 'Welcome', $title , 'edit_theme_options', 'accesspressroot-welcome', array( $this, 'accesspressroot_welcome_screen' ));
			}

			/** Welcome Page **/
			public function accesspressroot_welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php /* translators: %1$s : theme name, %2$s : theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'accesspress-root' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
					<div class="about-text"><?php /* translators: %s : theme name */ printf( esc_html__( 'The %s is a simple, clean, beautifully designed responsive WordPress business theme with drag and drop homepage sections. Its minimal but mostly used features will help you setup your website easily and quickly. Full width and boxed layout, featured slider, featured posts, services/features/projects layout, testimonial layout, blog layout, social media integration, call to action and many other page layouts.', 'accesspress-root' ), esc_html($this->theme_name) ); ?></div>
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo esc_url(admin_url('themes.php?page=accesspressroot-welcome&section='.$id)); ?>" class="<?php echo esc_attr($nav_class); ?>" >
			...
			public function accesspressroot_welcome_styles_and_scripts() {
				...
				wp_localize_script( 'accesspress-basic-welcome-screen', 'accesspressrootWelcomeObject', array(
					'admin_nonce'	=> wp_create_nonce('accesspressroot_plugin_installer_nonce'),
					'activate_nonce'	=> wp_create_nonce('accesspressroot_plugin_activate_nonce'),
					'ajaxurl'		=> esc_url( admin_url( 'admin-ajax.php' ) ),
					'activate_btn' => __('Activate', 'accesspress-root'),
					'installed_btn' => __('Activated', 'accesspress-root'),
					'demo_installing' => __('Installing Demo', 'accesspress-root'),
					'demo_installed' => __('Demo Installed', 'accesspress-root'),
					'demo_confirm' => __('Are you sure to import demo content ?', 'accesspress-root'),
				) );
			}

			/** Plugin API **/
			public function accesspressroot_call_plugin_api( $plugin ) {
			...
			/** Check For Icon **/
			public function accesspressroot_check_for_icon( $arr ) {
				if ( ! empty( $arr['svg'] ) ) {
			...
			public function accesspressroot_plugin_active($plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
				$status = 'install';
			...
			public function accesspressroot_plugin_generate_url($status, $plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
			...
			/* ========== Plugin Installation Ajax =========== */
			public function accesspressroot_plugin_installer_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'accesspress-root' ) );
				...
				if (! wp_verify_nonce( $nonce, 'accesspressroot_plugin_installer_nonce' ))
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'accesspress-root') );
				...
				// Get Plugin Info
				$api = $this->accesspressroot_call_plugin_api($plugin);
			...
			public function accesspressroot_plugin_offline_installer_callback() {
			...
			public function accesspressroot_plugin_offline_activation_callback() {
			...
			/** Plugin Activation Ajax **/
			public function accesspressroot_plugin_activation_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to activate plugins on this site.', 'accesspress-root' ) );

				$nonce = isset( $_POST["nonce"] ) ? sanitize_text_field( wp_unslash( $_POST["nonce"] ) ) : '';
				$plugin = isset( $_POST["plugin"] ) ? sanitize_text_field( wp_unslash( $_POST["plugin"] ) ) : '';

				// Check our nonce, if they don't match then bounce!
				if (! wp_verify_nonce( $nonce, 'accesspressroot_plugin_activate_nonce' ))
					die( esc_html__( 'Error - unable to verify nonce, please try again.', 'accesspress-root' ) );
				...
				// Get Plugin Info
				$api = $this->accesspressroot_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'accesspress-root');
				}
			...
			public static function get_plugin_file( $plugin_slug ) {
	      	...
	      	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=accesspressroot-welcome&section=import_demo'),'accesspressroot-file-installation');
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'accesspress-root' => array(
				'title' => __('Root Demo', 'accesspress-root'),
				'name' => 'accesspress-root',
				'screenshot' => get_template_directory_uri().'/welcome/demos/accesspress-root/screen.png',
				'home_page' => 'home',
				'menus' => array(
		...
		$demoimporter->options_replace_url = 'http://demo.accesspressthemes.com/accesspress-root'; // Set the url to be replaced with current siteurl
		$demoimporter->option_name = 'accesspress-root'; // Set the the name of the option if the theme is based on theme option
	endif;
?>